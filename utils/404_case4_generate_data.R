##############################################################################
## Case example 4 Data generation
##############################################################################
...
d/dt(eff) = Kin*(1+ PL + Emax*Concentration/(EC50 + Concentration)) - Kout*(1)*eff;
"
...
set.seed(12345666)
ndose <- 3
nsub <- 80 # number of subjects per dose
nstud <- 1 # number of studies

subgroup <- rep(rep(c(rep(0,ceiling(nsub/2)),rep(1,floor(nsub/2))),ndose),nstud)
WT0 <- runif(nsub*ndose*nstud,80,150)
VCOV <- 1
CLCOV <- 1

CL <- 15*exp(rnorm(nsub*ndose*nstud,0,.05))*CLCOV
KA <- 10
V2 <- 65*VCOV

Kout = 0.008/7
Kin <- WT0*Kout
PL <- 0.02+rnorm(nsub*ndose*nstud,0,0.04) + subgroup*-0.05
EMAX <- -0.04
DOSE <- rep(sort(rep(c(0,50,150),nsub)), nstud)
STUDY <- sort(rep(c(1:nstud), ndose*nsub))

theta.all <- 
  cbind(KA=KA, CL=CL, V2=V2,
        Q=7.4*CLCOV, V3=150*VCOV, 
        Kin=Kin, Kout=Kout, EC50 = 0.018, PL = PL, Emax = EMAX, isPRED = 0, WT0 = WT0)

# Nominal times for sampling schedule
SAMPLING <- c(c(-24,-0.1,0.1,0.5,1,2,4,8,12,18,23.9),23.9+c(1,2,3,4,6,8,12)*7*24,
              84*24 + c(0.1,0.5,1,2,4,8,12,18,23.9,36,47.9,71.9,95.9))
DOSING <- seq(0,168*24,24)
...
for(i in 1:length(ev.all)){
  ...
  inits <- c(depot = 0, centr = 0, peri = 0, eff=as.numeric(theta["WT0"]))
...
add_err <- rnorm(length(x.all.df$eff),0,0.6)
...
PKPDdataset <- my.data[,c("ID","time","NT","amt","DV","CMT","CMT_label","BLQ","evid","WT0","DOSE_label","DOSE","DAY","DAY_label","CYCLE","PART","STUDY","IPRED", "subgroup")]
...
PKPDdataset2 <- PKPDdataset2[,c("ID","time","NOMTIME","TIMEUNIT","amt","DV","CMT",
                                "CMT_label","EVENTU","BLQ","evid","WT0","DOSE_label",
                                "DOSE","DAY","PROFTIME","CYCLE","PART","STUDY","IPRED", "subgroup")]
names(PKPDdataset2) <- c("ID","TIME","NOMTIME","TIMEUNIT","AMT","LIDV","CMT",
                         "NAME","EVENTU","CENS","EVID","WEIGHTB","TRTACT",
                         "DOSE","PROFDAY","PROFTIME","CYCLE","PART","STUDY","IPRED", "subgroup")

write.csv(PKPDdataset2,"./data/404_case4_PKPDdataset.csv",row.names = FALSE)

# DT::datatable(PKPDdataset2, rownames = FALSE, options = list(autoWidth = TRUE, scrollX=TRUE) )
