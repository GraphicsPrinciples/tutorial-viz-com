...
gg2 <- my_data %>%
  ...
  scale_colour_brewer(palette = "Set2" , 
                      name = expression(paste("AUC0-24h (h*",mu,"g/mL)", sep = ""))) +
  scale_y_continuous(breaks = seq(-800,800,200)) +
  paper_theme()  +
  theme(axis.title.x = element_text(size = 9),
        axis.title.y = element_text(size = 9),
...
gg3 <- grid.arrange(textGrob("Exposure is not a better predictor of response than dose", 
                             gp = gpar(fontsize = 14), hjust = 0.6),
